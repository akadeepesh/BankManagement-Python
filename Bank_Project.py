...
try:
    ...
    os.system('cmd /c "pip install pyttsx3==2.7"')
try:
    import pyglet
except ImportError as e:
...
'''----------------------------------------------------------------------------------------------------'''
#Ascii art print
'''a = (int(width/2)-100)/15
b = int(a) * "‎*"'''
tprint("‎DC", font="doom")
print(14*"=",">")
engine.say("Welcome to Delhi corporation")
engine.runAndWait()
'''----------------------------------------------------------------------------------------------------'''
...
background_image=ImageTk.PhotoImage(file="sql.png")
...
e=Entry(frame, width=20,bg="greenyellow",fg="grey5",relief=SUNKEN)
e.grid(row=0,column=1,padx=20,pady=20)
e.insert(0,"root")
l1=Label(frame1,text="PASSWORD:",bg="grey5",fg="cyan").grid(row=0,column=0,pady=20)
f=Entry(frame1, width=20,bg="greenyellow",fg="grey5",relief=SUNKEN,show="\a")
f.grid(row=0,column=1,padx=20,pady=20)
f.insert(0,"deepesh")
btn=PhotoImage(file='submit.png' )
#img=Label(image=btn)
'''----------------------------------------------------------------------------------------------------'''
def load():
...
def conn():
    ...
    cd=f.get()
    try:
        mydb=mysql.connector.connect(host="localhost",user=ab,passwd=cd)
    except:
        root.destroy()
        print("\nSoory But You Entered Wrong Username or Password,\nI Can't give you Acsess To Do Any Transactions")
        print("\nYou Entered(",cd,")Password Kindly Recheck That")
        sys.exit()
    root.destroy()
    ...
    mydb.commit()
    commands = ["Enter Account Number","Enter Name (Limit 35 Characters)","Enter City Name","Enter Mobile Number",
                "Enter Amount To Be Deposited","Enter Amount To Be Withdrawn"]
    colon = ":"
    while(True):
        ...
        if(ch==1):
            ...
            mycursor.execute("select exists(select * from bank_master where acno='"+acno+"')")
            a=mycursor.fetchall()
            mycursor.execute("select exists(select * from bank_master where mobileno='"+mn+"')")
            b=mycursor.fetchall()
            for x in a:
                        ...
                        if(y[0]==1):
                            ...
                            for x in a:
                                if(x[0]+30>=wd):
                                    ...
                                    mydb.commit()
                                    print("You Have Withdrawn",wd,"Rupees From",acno,"Account\n")
                                else:
                                    print("You Can't Withdraw From This Amount (Minimum Amount Exceeds)\n")
                        else:
                            print("Mobile No. Is Incorrect\n")
                    else:
                        print("Account Dosent Exists\n")
            ...
            acno1=str(input("Enter Account Number In Which You Are Transfering Money :"))
            mycursor.execute("select exists(select * from bank_master where acno='"+acno+"')")
            a=mycursor.fetchall()
            mycursor.execute("select exists(select * from bank_master where acno='"+acno1+"')")
            b=mycursor.fetchall()
            for x in a:
                print("Checking Account No....")
                time.sleep(2)
                for y in b:
                    ...
                    if(x[0]==1):
                        if(y[0]==1):
                            ...
                            mycursor.execute("insert into banktrans values('"+acno+"','"+str(dp)+"','"+dot+"','"+ttype+"')")
                            mycursor.execute("update bank_master set balance=balance-'"+str(dp)+"' where acno='"+acno+"'")
                            mycursor.execute("update bank_master set balance=balance+'"+str(dp)+"' where acno='"+acno1+"'")
                            mydb.commit()
                            print("Money Has Been Transferd Successully!!!\n")
                        else:
                            print("Reciving Account Dosent Exists\n")
                    else:
                        print("Account From Which Money Have To Transfer Dosent Exists\n")
        elif(ch==6):
            if(input("Enter The Password To See Accounts  :")=="SDMS-DSH"):
                ...
                print("Contact Deepesh At deepesh.025012@kvsrodelhi.in For The Correct Password\n")
        else:
            root2=Tk()
